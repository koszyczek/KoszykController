...
public class PlayerScript : Script
{
    ...
    // Add a normal speed variable
    public float NormalSpeed = 400;
    // Add a FOV for sprinting
    public float SprintFOV = 75f;
    // Add a normal FOV
    public float NormalFOV = 60f;
    // Add a variable to control the speed of FOV change
    ...
    public override void OnUpdate()
    {
        ...
        {
            // Increase the max velocity to sprint speed
            MaxVelocityGround = SprintSpeed;
            MaxVelocityAir = SprintSpeed;
            // Gradually change the FOV to the sprint FOV
            Camera.FieldOfView = Mathf.Lerp(Camera.FieldOfView, SprintFOV, Time.DeltaTime * FOVSpeed);
        }
        else
        {
            // Reset the max velocity to normal speed
            MaxVelocityGround = NormalSpeed;
            MaxVelocityAir = NormalSpeed;
            // Gradually change the FOV back to the normal FOV
            Camera.FieldOfView = Mathf.Lerp(Camera.FieldOfView, NormalFOV, Time.DeltaTime * FOVSpeed);
        }
    ...
    public override void OnFixedUpdate()
    {
        ...
        if (_jump && PlayerController.IsGrounded)
        {
            ...
            // Reset the vertical velocity after jumping
            _velocity.Y = 0;
        }
        _jump = false;
